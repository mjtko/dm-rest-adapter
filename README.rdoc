...
Note the :scheme and :port are optional.
...
relationships.  For example, you can map back to a parent as a nested resource
(I'm not sure what the semantics of this would be if you then fetched the child
from that nested resource again, and so on...)
...
such scenarios on the service-side.  Though DataMapper REST Adapter will filter
responses that are returned from the service, it will provide hints via the
